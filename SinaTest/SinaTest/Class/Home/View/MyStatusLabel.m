...
#define  MyLinkBackgroundTag 100
...
- (NSMutableArray *)links {
    if (!_links) {
        ...
        [self.attributedText enumerateAttributesInRange:NSMakeRange(0, self.attributedText.length) options:0 usingBlock:^(NSDictionary *attrs , NSRange range , BOOL *stop){
            ...
            NSString *linkText = attrs[MyLinkAttr];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (touchingLink) {
        // 说明手指在某个链接上面抬起来, 发出通知
        [[NSNotificationCenter defaultCenter] postNotificationName:MyLinkDidSelectedNotification object:nil userInfo:@{MyLinkAttr : touchingLink.text}];
    }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (touchingLink) {
        ...
        //触摸普通文本
        [[NSNotificationCenter defaultCenter] postNotificationName:MyStatusNormalTextDidSelectedNotification object:nil];
    }
...
- (void)showLinkBackground:(MyLink *)link {
    for (UITextSelectionRect *selectionRect in link.rects) {
        ...
        bg.backgroundColor = MyColor(177, 214, 255);
