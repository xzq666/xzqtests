...
- (void)showToView{
    ...
    self.disMissBlock = ^(NSInteger page){
        ...
        if(weakSelf.status == UIViewAnimationAnimationStatusZoom){
            ...
            if ([weakSelf isDataSourceElsePhotos]) {
                toImageView2 = (UIImageView *)[[self.dataSource photoBrowser:self photoAtIndexPath:[NSIndexPath indexPathForItem:page inSection:self.currentIndexPath.section]] toView];
                NSLog(@"-->%@",toImageView2);
            }else{
