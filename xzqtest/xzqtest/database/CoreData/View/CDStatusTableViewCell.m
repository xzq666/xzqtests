...
@interface CDStatusTableViewCell(){
...
@implementation CDStatusTableViewCell
...
-(void)setStatus:(CDStatus *)status{
    ...
    _avatar.image=[UIImage imageNamed:status.cduser.profileImageUrl];
    ...
    //根据文本内容取得文本占用空间大小
    CGSize userNameSize=[status.cduser.name sizeWithAttributes:@{NSFontAttributeName: [UIFont systemFontOfSize:kStatusTableViewCellUserNameFontSize]}];
    CGRect userNameRect=CGRectMake(userNameX, userNameY, userNameSize.width,userNameSize.height);
    _userName.text=status.cduser.screenName;
    ...
    _mbType.image=[UIImage imageNamed:status.cduser.mbtype];
